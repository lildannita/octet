...
> ⚠️ **Внимание: возможна ошибка доступа к директории хранения данных**
>
> В текущей версии, при первом запуске контейнера Docker может автоматически создать директорию с правами, которые не подходят для работы приложения — это может привести к сбоям из-за отсутствия доступа к файлам.
>
> ✅ **Рекомендуется:** перед запуском контейнера вручную создать директорию и убедиться, что у неё корректные права.
